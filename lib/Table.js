...
class Table {
    ...
    createTable(table_data) {
        ...
        for(let col = 0; col < this.number_of_columns; col++) {
            ...
            for(let row = 0; row < this.number_of_rows; row++) {
                this.addRow(col, data[row][meta_data[col].name]);
            }
        }
    ...
    printTable() {
        ...
        if(this.number_of_rows > 0) {
            console.info(`\n${'-'.repeat(this.table_width)}`);

            let print_line = '|';
            this.columns.forEach((column) => print_line += column.printColumnName());
            ...
            for(let row = 0; row < this.number_of_rows; row++) {
                print_line = '|';
                this.columns.forEach((column) => print_line += column.printRow(row));
                console.info(print_line);
            }
        ...
        // Display message for no data
            ...
            let message = 'No records to display. Please add records before viewing them.';
            console.info(`\n${'-'.repeat(message.length + 4)}`);
            console.info(`| ${message} |`);
            console.info(`${'-'.repeat(message.length + 4)}\n`);
        }
    }
}
